...
function squeeze_excite(channels, reduction = 4)
  ...
  SkipConnection(Chain(AdaptiveMeanPool((1, 1)),
                       conv_bn((1, 1), channels, channels ÷ reduction, relu1; bias = false)...,
...
"""
function invertedresidual(kernel_size, inplanes, hidden_planes, outplanes, activation = relu1;
                          stride, reduction = nothing)
...
function mobilenetv2(width_mult, configs; max_width = 1280, nclasses = 1000)
  ...
  layers = []
  append!(layers, conv_bn((3, 3), 3, inplanes, relu1, stride = 2))

  # building inverted residual blocks
  for (t, c, n, s) in configs
    outplanes = _round_channels(c * width_mult, width_mult == 0.1 ? 4 : 8)
    for i in 1:n
      push!(layers, invertedresidual(3, inplanes, inplanes * t, outplanes, relu1;
                                     stride = i == 1 ? s : 1))
      inplanes = outplanes
    end
  end
  ...
  return Chain(Chain(layers...,
                     conv_bn((1, 1), inplanes, outplanes, relu1, bias = false)...),
